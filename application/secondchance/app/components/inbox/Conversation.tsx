...
const Conversation: React.FC<ConversationProps> = ({ conversation, userId }) => {
  ...
  return (
    <div className="cursor-pointer rounded-xl border border-gray-300 px-6 py-4 transition hover:bg-gray-100">
      <p className="mb-6 text-xl">{otherUser?.name}</p>

      <p onClick={handleGoToConversation} className="text-secondchance-dark underline">
        Go to conversation
      </p>
    </div>
  );
